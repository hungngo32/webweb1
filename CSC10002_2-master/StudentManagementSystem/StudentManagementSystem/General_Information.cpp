...
{
	int idx = findValue(arr, numOfElements, sizeItem, val, equalValue);

	if (idx == NOT_FOUND)
		return arr;

	return deleteElementAtPosition(arr, numOfElements, sizeItem, idx, alloc, copyElement, release);
}
	...
	void (*copyElement)(void*, void*), void (*release)(void*, const int&))
{
	...
	void* newArray = alloc(numOfElements - 1);
	int j = 0;

	for (int i = 0; i < numOfElements; i++)
		if (i != pos)
		{
			copyElement((char*)newArray + (j * sizeItem), (char*)arr + (i * sizeItem));
			j++;
		}

	release(arr, numOfElements);
	--numOfElements;
